...
def get_content():
    ...
    for c in range(0, 10):
        ...
        if sj_test != Nowaday:
            conn = MySQLdb.connect(host='localhost', user='root', passwd='你的密码', db='你的数据库名', charset='utf8')
            cursor = conn.cursor()
           ...
           # cursor.execute("""create table if not exists cai_article(author int, `time` int, edittime int, eidtor char(100), `from` char(100), fromurl char(100), `type` int, thumb char(100), top int, click int, realclick int, title char(100), content text)""")
            ...
            value = (1, int_time(sj_now), int_time(Nowaday1), 'xxxx', 'xxxx', "http://uzone.univs.cn/blog/" + urls[c], 0, '0', 0, num1, 0, tm_json, zw_json)
            try:
